...
- `index-D0H6xQ20.js`: 1,142.72 kB (gzip: 291.51 kB) ⚠️ >500 kB warning

**Build Time**: 2.65s

**Improvement**: -0.67 kB from vite 7.1.12→7.2.0 upgrade in Iteration 92

## Size Regression Thresholds

| Threshold | Size | Action |
|-----------|------|--------|
| ⚠️ Warning | 1,200 kB | Investigate what caused growth |
| 🚨 Critical | 1,300 kB | Block release, require optimization |
| ✅ Target | <1,150 kB | Ideal range (current baseline) |

**Gzip Threshold** (user download size):
- ⚠️ Warning: >320 kB gzipped
- 🚨 Critical: >350 kB gzipped
- ✅ Current: 291.51 kB (good)

## Historical Data

| Iteration | Date | Total Size | JS Size | CSS Size | Change | Notes |
|-----------|------|------------|---------|----------|--------|-------|
| 92 | 2025-11-05 | 1,147.79 kB | 1,142.72 kB | 3.83 kB | -0.67 kB | vite 7.1.12→7.2.0 upgrade, improved build optimization |
...
- +117 tests (110 → 227 in Iterations 1-90)
...
- name: Check bundle size
  run: |
    ...
    if (( $(echo "$SIZE > 1200" | bc -l) )); then
      echo "⚠️ Warning: Bundle size $SIZE kB exceeds 1,200 kB threshold"
      exit 1
    fi
```
