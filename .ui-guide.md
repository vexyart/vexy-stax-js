...
All keyboard shortcuts are documented in main.js (`keydownHandler` function) and exposed via `window.vexyStax.help()`.

### Export & File Operations

| Key | Action | Description |
|-----|--------|-------------|
| `E` | Export PNG | Export current view as PNG (1x resolution) |
| `Shift+E` | Export PNG 2x | Export at 2x resolution (high-DPI) |
| `Ctrl+E` / `Cmd+E` | Export PNG 4x | Export at 4x resolution (print quality) |
| `J` | Export JSON | Save current configuration + images as JSON |
| `L` | Load JSON | Import JSON configuration |
| `C` | Copy to Clipboard | Copy current view to clipboard |
| `V` | Paste from Clipboard | Paste image from clipboard |
| `Delete` / `Backspace` | Delete Selected | Remove currently selected image |
| `Shift+Delete` | Clear All | Remove all images from stack |

### Camera Controls

| Key | Action | Description |
|-----|--------|-------------|
| `1` | Front View | Animate to front viewpoint |
| `2` | Top View | Animate to top viewpoint |
| `3` | Beauty View | Animate to beauty (hero shot) viewpoint |
| `4` | Side View | Animate to side viewpoint |
| `5` | 3D Stack View | Animate to 3D isometric view |
| `6` | Center View | Center camera on stack |
| `7` | Isometric View | Animate to isometric viewpoint |
| `R` | Reset Camera | Reset camera to default position |
| `F` | Center on Content | Center camera on loaded images |
...
| Key | Action | Description |
|-----|--------|-------------|
| `P` | Perspective Mode | Switch to perspective camera |
| `O` | Orthographic Mode | Switch to orthographic camera |
| `I` | Isometric Mode | Switch to isometric camera |
| `T` | Telephoto Mode | Switch to telephoto camera (narrow FOV) |

### Stack Manipulation

| Key | Action | Description |
|-----|--------|-------------|
| `↑` | Increase Z-Spacing | Move images further apart (Z-axis) |
| `↓` | Decrease Z-Spacing | Move images closer together |
| `[` | Decrease FOV | Narrow field of view (perspective mode) |
| `]` | Increase FOV | Widen field of view (perspective mode) |

### Settings

| Key | Action | Description |
|-----|--------|-------------|
| `S` | Save Settings | Save current settings to localStorage |
| `Shift+S` | Load Settings | Load settings from localStorage |
| `Ctrl+Z` / `Cmd+Z` | Undo | Undo last change (10 history states) |
| `Ctrl+Y` / `Cmd+Y` | Redo | Redo last undone change |
| `Shift+R` | Reset Settings | Reset all settings to defaults |

### Utility

| Key | Action | Description |
|-----|--------|-------------|
| `H` / `?` | Help | Show keyboard shortcut reference |
| `G` | Toggle Grid | Show/hide floor grid |
| `A` | Toggle Ambient Mode | Enable/disable ambient lighting mode |
| `D` | Toggle Debug | Show/hide FPS counter |
| `Esc` | Close Modals | Close any open dialogs/modals |

### Modifier Keys

- **Shift**: Activates alternative action (e.g., Shift+E for 2x export)
- **Ctrl/Cmd**: System-level actions (save, undo, redo)
- **Alt/Option**: Reserved for future features
