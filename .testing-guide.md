...
```
tests/
├── api_input_validation.test.js       # Public API input sanitization (10 tests)
├── core_app_state.test.js            # AppState lifecycle & operations (16 tests)
├── core_constants.test.js            # Constants validation (21 tests)
├── core_event_bus.test.js            # EventBus pub/sub patterns (16 tests)
├── core_integration.test.js          # Cross-module integration (5 tests)
├── core_ordering.test.js             # Array reordering utilities (8 tests)
├── core_render_loop.test.js          # Animation loop & FPS monitoring (20 tests)
├── core_shared_state.test.js         # Shared state registry (12 tests)
├── core_studio_sizing.test.js        # Retina/DPR calculations (9 tests)
├── error_recovery.test.js            # Manager cleanup & dispose (23 tests)
├── managers_camera_animation.test.js # Camera animation (10 tests)
├── managers_floor.test.js            # Floor creation/removal (7 tests)
├── managers_lighting.test.js         # Lighting setup (5 tests)
├── managers_scene.test.js            # Scene initialization (8 tests)
├── utils_helpers.test.js             # Helper functions (28 tests)
└── utils_logger.test.js              # Logger utilities (4 tests)
```
...
- ✅ `updates FPS display when enabled`
- ❌ `test function` (too vague)
...
**Total Duration**: 627.0ms ±3.2ms (223 tests)

**Thresholds**:
- ⚠️ Warning: >700ms (+11% from baseline)
- 🚨 Critical: >800ms (+27% from baseline)
